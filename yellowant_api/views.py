...
def yellowant_api(request):
    ...
    if data["verification_token"] == settings.YA_VERIFICATION_TOKEN:
        cc = CommandCenter(data["user"], data['application'], data['function_name'], data['args'],data['application_invoke_name'])
        reply = cc.parse()
        # print (reply)
...
def api_key(request):
    ...
    try:
        ec2 = boto3.resource(service_name='ec2', region_name="us-east-2", api_version=None,
